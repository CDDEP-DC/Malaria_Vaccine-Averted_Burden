# Map of deaths averted by country for Malaria Vaccine-Averted Burden
...
AfrShape@data$Deaths_averted_pt <- AfrShape@data$D_VE1_avd / AfrShape@data[,"2011"] * 1000 # Calculate density (per thousand persons)
AfrShape@data$Deaths_averted_pt_min <- AfrShape@data$D_VE1_avd_min / AfrShape@data[,"2011"] * 1000
AfrShape@data$Deaths_averted_pt_max <- AfrShape@data$D_VE1_avd_max / AfrShape@data[,"2011"] * 1000
...
Deaths_avertedplot <- ggplot() + 
  ...
  ggtitle( "Deaths Averted per 1,000 Population by Country, WHO Africa Region (2021-2030)") +
  theme_void() +
